...
func (slf *stuClientIP) getClientIP(fiberCtx *fiber.Ctx) string {
	...
	if ip, err := slf.fromSpecialHeaders(fiberCtx); err == nil {
		return ip
	}

	if ip, err := slf.fromForwardedHeaders(fiberCtx); err == nil {
		return ip
	}
...
func (slf *stuClientIP) fromSpecialHeaders(fiberCtx *fiber.Ctx) (string, error) {
	ctx := fiberCtx.Context()
...
func (slf *stuClientIP) fromForwardedHeaders(fiberCtx *fiber.Ctx) (string, error) {
	ctx := fiberCtx.Context()
...
func (slf *stuClientIP) isPrivateAddress(address string) (bool, error) {
...
func (slf *stuClientIP) retrieveForwardedIP(forwardedHeader string) (string, error) {
